    ...
    <AppCard>
      ...
      >
        ...
        <template #cell-store="{ item }">
          <span v-if="item.store" class="font-medium text-text-primary">
            {{ item.store.name }}
          </span>
        ...
        <template #cell-treasury="{ item }">
          <span v-if="item.treasury" class="font-medium text-text-primary">
            {{ item.treasury.name }}
          </span>
          <span v-else class="text-text-muted text-sm">—</span>
        </template>
        ...
        <template #cell-bank="{ item }">
          <span v-if="item.bank" class="font-medium text-text-primary">
            {{ item.bank.name }}
          </span>
          <span v-else class="text-text-muted text-sm">—</span>
        </template>
...
// تعريف أعمدة الجدول بعد إضافة حقول الربط اللوجستي والمالي
const tableHeaders = [
  { key: 'id', label: '#' },
  { key: 'full_name', label: 'الاسم الكامل' },
  { key: 'username', label: 'اسم المستخدم' },
  { key: 'role', label: 'الدور' },
  { key: 'store', label: 'المخزن الافتراضي' },
  { key: 'treasury', label: 'الخزنة الافتراضية' },
  { key: 'bank', label: 'البنك الافتراضي' },
